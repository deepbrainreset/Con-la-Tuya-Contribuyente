...
export default function Navbar({ currentTab, onNavigate, visits }: NavbarProps) {
  ...
  const menuItems = [
    ...
    { id: 'tributos', label: 'Tributos', icon: FileText },
    { id: 'inflacion', label: 'Inflación', icon: LineChart },
    { id: 'responsabilidad-politica', label: 'Responsabilidad', icon: Vote },
  ...
  return (
    <nav className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur-md border-b border-slate-800" id="global-navbar">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3">
            <button onClick={() => handleNavClick('landing')} className="flex items-center gap-2 group cursor-pointer text-left">
              ...
              <div>
                <span className="font-extrabold text-white tracking-tight block text-base leading-none">Con La Tuya</span>
                <span className="text-[10px] text-emerald-400 font-mono tracking-widest block uppercase">Contribuyente</span>
              </div>
            </button>
          ...
          <div className="2xl:hidden flex items-center">
            <button type="button" onClick={() => setMobileOpen(!mobileOpen)} className="inline-flex items-center justify-center p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-950 focus:outline-none" aria-label="Toggle menu">
      ...
      {mobileOpen && (
        <div className="2xl:hidden border-t border-slate-900 bg-slate-950 px-2 pt-2 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-1">
          {menuItems.map((item) => {
            const Icon = item.icon;
            const isActive = currentTab === item.id || currentTab.startsWith(item.id + '/');
            return (
