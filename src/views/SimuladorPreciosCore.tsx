...
import {
  ShoppingBag, ChevronRight, Info, AlertTriangle, Layers3, ReceiptText, Repeat2,
  Landmark, Factory, TrendingUp, PieChart, Search, CircleCheck, CircleDashed
} from 'lucide-react';
...
import { GENERIC_PRODUCT_CATALOG, GenericProductCatalogItem, normalizeProductSearch } from '../data/catalogoProductos';
...
export default function SimuladorPreciosCore() {
  ...
  const [pendingProduct, setPendingProduct] = useState<GenericProductCatalogItem | null>(null);

  const product = PRODUCTS.find(p => p.id === selectedProductId) || PRODUCTS[0];
  const chain = PRODUCT_TAX_CHAINS.find(c => c.productId === selectedProductId);
  const baseTotal = product.basePrice + product.logistics + product.taxNational + product.taxProvincial + product.taxMunicipal + product.margin;

  const filteredCatalog = useMemo(() => {
    const q = normalizeProductSearch(searchQuery);
    const ranked = GENERIC_PRODUCT_CATALOG.filter(item => {
      if (!q) return true;
      const haystack = normalizeProductSearch([item.name, item.category, ...item.aliases].join(' '));
      return haystack.includes(q);
    }).sort((a, b) => {
      if (!!a.modelProductId !== !!b.modelProductId) return a.modelProductId ? -1 : 1;
      return a.name.localeCompare(b.name, 'es');
    });
    return ranked.slice(0, q ? 30 : 14);
  }, [searchQuery]);

  const selectCatalogItem = (item: GenericProductCatalogItem) => {
    if (item.modelProductId) {
      setSelectedProductId(item.modelProductId);
      setUserPriceMultiplier(1);
  ...
  const values = useMemo(() => ({
    total: baseTotal * userPriceMultiplier,
  ...
  }), [product, userPriceMultiplier, baseTotal]);
    ...
    <div className="space-y-8 py-4 text-left" id="simulador-precios-core-view">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <aside className="lg:col-span-4 space-y-4 min-w-0">
          <div>
            <span className="text-[10px] text-slate-500 font-mono tracking-wider block uppercase pl-1">Buscar producto o servicio</span>
            <p className="text-[11px] text-slate-400 mt-1">Buscá por nombre genérico, sin marca: leche, taladro, heladera, zapatillas, celular, combustible, internet, etc.</p>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Ej.: taladro, arroz, heladera..."
              className="w-full rounded-xl border border-slate-700 bg-slate-950 py-3 pl-10 pr-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-emerald-500/60"
            />
          </div>
          ...
          <div className="max-h-[430px] overflow-y-auto space-y-2 pr-1">
            {filteredCatalog.length === 0 ? (
              <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-300">
                No encontramos ese término en el catálogo genérico todavía. Esto no significa que el producto no exista: sólo que falta incorporarlo al nomenclador.
              </div>
            ) : filteredCatalog.map(item => {
              const modeled = !!item.modelProductId;
              const selected = modeled && item.modelProductId === selectedProductId && !pendingProduct;
              return (
                <button
                  key={item.id}
                  onClick={() => selectCatalogItem(item)}
                  className={`w-full flex items-center justify-between gap-3 p-3.5 text-left rounded-xl border transition cursor-pointer ${selected ? 'bg-emerald-500/10 border-emerald-500/35 text-white' : 'bg-slate-900 border-slate-800 text-slate-300 hover:text-white hover:border-slate-700'}`}
                >
                  <div className="flex items-start gap-3 min-w-0">
                    <ShoppingBag className={`w-4 h-4 shrink-0 mt-0.5 ${modeled ? 'text-emerald-400' : 'text-slate-400'}`} />
                  ...
                  <div className="shrink-0 flex items-center gap-1 text-[9px] font-mono">
                    {modeled ? <><CircleCheck className="w-3.5 h-3.5 text-emerald-400" /><span className="text-emerald-300">Calculable</span></> : <><CircleDashed className="w-3.5 h-3.5 text-amber-300" /><span className="text-amber-200">Pendiente</span></>}
                  </div>
                </button>
              );
            })}
          ...
          {!pendingProduct && (
            <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
              <label className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block">Precio que pagaste en góndola</label>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  min="1"
                  step="1"
                  value={Math.round(values.total)}
                  onChange={e => {
                    const price = Number(e.target.value);
                    if (Number.isFinite(price) && price > 0 && baseTotal > 0) {
                      setUserPriceMultiplier(Math.max(0.01, price / baseTotal));
                    }
                  }}
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-base sm:text-sm font-mono text-white focus:outline-none focus:border-emerald-500/60"
                />
              </div>
              <p className="text-[10px] text-slate-400 leading-relaxed">El desglose se escala al precio ingresado manteniendo las proporciones del modelo disponible. Esto no convierte una estimación en un dato auditado del comercio concreto.</p>
            </div>
          )}
        </aside>
        ...
        <section className="lg:col-span-8 space-y-6 min-w-0">
          {pendingProduct ? (
            <div className="bg-amber-500/5 border border-amber-500/25 rounded-2xl p-5 sm:p-6 space-y-4">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                <div>
                  <h2 className="font-bold text-white text-lg">{pendingProduct.name}</h2>
                  <p className="text-xs text-slate-300 mt-1">{pendingProduct.category}</p>
                </div>
              </div>
              <p className="text-sm text-amber-100/90 leading-relaxed">
                El producto ya está incluido en el buscador nacional, pero todavía no tiene un modelo fiscal suficientemente verificado para calcular cuánto del precio de góndola corresponde a Nación, Provincia y Municipio.
              </p>
              <p className="text-xs text-slate-300 leading-relaxed">
                No vamos a copiar los porcentajes de otro producto ni asignarle automáticamente IVA, IIBB o tasas locales sin verificar actividad, jurisdicción, exenciones y régimen aplicable. Cuando esa ficha se complete, este mismo resultado pasará a ser calculable.
              </p>
            </div>
          ) : <>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 grid grid-cols-2 md:grid-cols-4 gap-4"><Metric title="Precio góndola" value={money(values.total)} note="Precio ingresado / modelado." tone="emerald" /><Metric title="Costo productivo + logística" value={money(modeledProductionCost)} note={`${pct(productionShare)} del precio final.`} /><Metric title="Impuestos + tasas" value={money(totalTaxes)} note={`${pct(taxShare)} del precio final.`} tone="rose" /><Metric title="Margen comercial bruto" value={money(values.margin)} note={`${pct(marginShare)} del precio final.`} tone="emerald" /></div>
