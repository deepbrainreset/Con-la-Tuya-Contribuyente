    ...
    <div className="space-y-8 py-4 text-left" id="simulador-precios-view">
      <div className="border-b border-slate-800 pb-4">
        ...
        <p className="text-xs text-slate-400 mt-1 max-w-4xl leading-relaxed">
          Visualizá qué parte del precio final corresponde al costo productivo/logístico modelado, cuánto corresponde a margen comercial y cuánto agregan impuestos y tasas nacionales, provinciales y municipales. Si falta evidencia para una etapa o jurisdicción, se informa como “Sin datos verificados”.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <Metric title="Precio góndola" value={money(values.total)} note="100% del precio modelado." tone="emerald" />
            <Metric title="Costo productivo + logística" value={money(modeledProductionCost)} note={`${pct(productionShare)} del precio final.`} />
            <Metric title="Impuestos + tasas" value={money(totalTaxes)} note={`${pct(taxShare)} del precio final.`} tone="rose" />
            <Metric title="Margen comercial bruto" value={money(values.margin)} note={`${pct(marginShare)} del precio final.`} tone="emerald" />
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <div className="flex items-center gap-2"><PieChart className="w-4 h-4 text-emerald-400" /><h2 className="font-bold text-white">Composición del precio final de góndola</h2></div>
            <p className="text-[11px] text-slate-500 leading-relaxed">Lectura directa sobre el 100% del precio. “Costo productivo” es el costo modelado de producción + logística disponible en la base; no se presenta como costo contable auditado.</p>
            <div className="space-y-3">
              <CompositionRow label="Costo productivo + logística" amount={modeledProductionCost} share={productionShare} />
              <CompositionRow label="Impuestos nacionales" amount={values.national} share={nationalShare} tone="rose" />
              <CompositionRow label="Impuestos provinciales" amount={values.provincial} share={provincialShare} tone="amber" />
              <CompositionRow label="Tasas / tributos municipales" amount={values.municipal} share={municipalShare} tone="sky" />
              <CompositionRow label="Margen comercial bruto" amount={values.margin} share={marginShare} tone="emerald" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
              <Metric title="Precio sin carga fiscal identificada" value={money(priceWithoutIdentifiedTaxes)} note="Costo modelado + margen." />
              <Metric title="Carga fiscal total" value={pct(taxShare)} note="Parte del precio de góndola explicada por impuestos/tasas." tone="rose" />
              <Metric title="Costo productivo dentro de góndola" value={pct(productionShare)} note="Parte del precio final atribuida al costo modelado." />
            </div>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
            <div className="flex items-center gap-2"><Factory className="w-4 h-4 text-emerald-400" /><h2 className="font-bold text-white">Cuánto encarece la carga fiscal sobre el costo del producto</h2></div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <Metric title="Nación / costo productivo" value={`+${pct(nationalUplift)}`} note={`${money(values.national)} agregados sobre el costo modelado.`} tone="rose" />
              <Metric title="Provincia / costo productivo" value={`+${pct(provincialUplift)}`} note={`${money(values.provincial)} agregados sobre el costo modelado.`} tone="rose" />
              <Metric title="Municipio / costo productivo" value={`+${pct(municipalUplift)}`} note={`${money(values.municipal)} agregados sobre el costo modelado.`} tone="rose" />
              <Metric title="Carga tributaria total / costo" value={`+${pct(taxUpliftOverProduction)}`} note={`${money(totalTaxes)} sobre producción + logística.`} tone="rose" />
              <Metric title="Suba total hasta góndola" value={`+${pct(shelfUpliftOverProduction)}`} note="Incluye impuestos/tasas + margen modelado." />
              <Metric title="Markup comercial / costo" value={pct(grossMarkupOnProduction)} note="Margen bruto modelado; no es utilidad neta." tone="emerald" />
            </div>
            <div className="flex gap-3 p-3 bg-amber-500/5 border border-amber-500/20 rounded-xl text-[11px] text-amber-100/80 leading-relaxed"><TrendingUp className="w-4 h-4 text-amber-400 shrink-0" /><p><strong>Potencial empresario:</strong> el margen bruto equivale a {pct(grossMarginShelf)} del precio final en este modelo. De ahí todavía pueden salir salarios administrativos, alquiler, financiación, amortizaciones, pérdidas, seguros, marketing, Ganancias y otros costos. No se etiqueta como beneficio neto.</p></div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            {[['Nación', values.national, nationalShare, nationalUplift], ['Provincia', values.provincial, provincialShare, provincialUplift], ['Municipio', values.municipal, municipalShare, municipalUplift]].map(([label, amount, share, uplift]) => <div key={String(label)} className="grid grid-cols-12 gap-3 items-center text-xs"><span className="col-span-3 text-slate-300 font-semibold">{label}</span><div className="col-span-4 h-2 bg-slate-950 rounded overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${Math.min(100, Number(share) * 2)}%` }} /></div><span className="col-span-2 text-right font-mono text-white">{money(Number(amount))}</span><span className="col-span-1 text-right font-mono text-slate-400">{pct(Number(share))}</span><span className="col-span-2 text-right font-mono text-rose-300">+{pct(Number(uplift))} s/costo</span></div>)}
          </div>
