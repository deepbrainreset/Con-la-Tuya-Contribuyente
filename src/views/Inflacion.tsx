...
const formatPct = (value: number | null) => value === null ? '—' : `${value.toLocaleString('es-AR', { maximumFractionDigits: 2 })}%`;
    ...
    <div className="space-y-8 py-4 text-left" id="inflacion-view">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/40 border border-amber-500/20 rounded-2xl p-5">
          ...
          <div className="text-3xl font-black text-white mt-2">2007–2015</div>
          <div className="text-xs text-slate-400 mt-1">Advertencia metodológica oficial de INDEC</div>
        </div>
        <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5">
          <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500 font-black">Brecha media en años comparados</span>
          <div className="text-3xl font-black text-white mt-2">{avgGap.toFixed(1)} p.p.</div>
          <div className="text-xs text-slate-400 mt-1">Alternativa menos serie oficial publicada</div>
        </div>
      </section>
      ...
      <section className="bg-slate-900/30 border border-slate-800 rounded-2xl p-4 sm:p-6 space-y-5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            ...
            <button onClick={() => setShowAlternative(v => !v)} className={`px-3 py-2 rounded-lg border cursor-pointer ${showAlternative ? 'border-amber-500/30 bg-amber-500/10 text-amber-300' : 'border-slate-800 text-slate-500'}`}>Privada / alternativa</button>
          </div>
        </div>

        <div className="w-full h-[420px] bg-slate-950/40 rounded-xl border border-slate-900 p-2 sm:p-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 12, right: 22, left: 0, bottom: 8 }}>
              ...
              <Line type="monotone" dataKey="alternativa" name="Privada / alternativa" stroke="#f59e0b" strokeWidth={2.5} strokeDasharray="6 4" dot={{ r: 3 }} connectNulls={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </section>
        ...
        {showTable && (
          <div className="overflow-x-auto border-t border-slate-800">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-950/60 text-[10px] uppercase tracking-wider font-mono text-slate-500">
                <tr>
                  <th className="px-4 py-3">Año</th>
                  <th className="px-4 py-3">Oficial / publicada</th>
                  <th className="px-4 py-3">Privada / alternativa</th>
                  <th className="px-4 py-3">Estado</th>
                  <th className="px-4 py-3">Fuentes</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-850">
                {INFLATION_SERIES.map(point => (
                  <tr key={point.year} className={point.evidence === 'oficial_con_reserva' ? 'bg-amber-500/[0.025]' : ''}>
                    <td className="px-4 py-3 font-mono font-bold text-white">{point.year}</td>
                    <td className="px-4 py-3">
                      <div className="font-bold text-emerald-300">{formatPct(point.official)}</div>
                      <div className="text-[10px] text-slate-500 mt-0.5 max-w-[240px]">{point.officialLabel}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="font-bold text-amber-300">{formatPct(point.alternative)}</div>
                      <div className="text-[10px] text-slate-500 mt-0.5 max-w-[240px]">{point.alternativeLabel || 'Sin una serie privada homogénea documentada para este año'}</div>
                    </td>
                    <td className="px-4 py-3">
                      {point.evidence === 'oficial_con_reserva' ? (
                        <span className="inline-flex px-2 py-1 rounded border border-amber-500/20 bg-amber-500/5 text-amber-300 font-mono text-[9px] uppercase">Con reserva</span>
                      ) : point.evidence === 'referencia' ? (
                        <span className="inline-flex px-2 py-1 rounded border border-slate-700 text-slate-400 font-mono text-[9px] uppercase">Transición</span>
                      ) : (
                        <span className="inline-flex px-2 py-1 rounded border border-emerald-500/20 bg-emerald-500/5 text-emerald-300 font-mono text-[9px] uppercase">Oficial</span>
                      )}
                      {point.note && <p className="text-[10px] text-slate-500 mt-2 max-w-xs leading-relaxed">{point.note}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1.5">
                        <a href={point.officialSource} target="_blank" rel="noreferrer" className="text-emerald-400 hover:underline inline-flex items-center gap-1">Fuente oficial/metodológica <ExternalLink className="w-3 h-3" /></a>
                        {point.alternativeSource && <a href={point.alternativeSource} target="_blank" rel="noreferrer" className="text-amber-400 hover:underline inline-flex items-center gap-1">Fuente alternativa <ExternalLink className="w-3 h-3" /></a>}
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="border border-slate-800 rounded-2xl p-5 bg-slate-900/20 space-y-3">
          <div className="flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-emerald-400" /><h3 className="font-bold text-white text-sm">2026: dato en curso</h3></div>
          <p className="text-2xl font-black text-white">{CURRENT_INFLATION_2026.lastMonthlyRate.toLocaleString('es-AR')}% <span className="text-xs font-normal text-slate-500">mensual en {CURRENT_INFLATION_2026.lastMonth}</span></p>
          <p className="text-xs text-slate-400 leading-relaxed">{CURRENT_INFLATION_2026.status} No se compara un parcial 2026 contra años completos.</p>
          <a href={CURRENT_INFLATION_2026.source} target="_blank" rel="noreferrer" className="text-xs text-emerald-400 hover:underline inline-flex items-center gap-1">Fuente INDEC <ExternalLink className="w-3 h-3" /></a>
        </div>
